...
import { PageHeader, Panel, Chip, LoadingCards } from "@/components/app/ui-kit";
...
function OperationsPage() {
  ...
  const counts = useQuery({
    ...
    queryFn: async () => {
      const [schedules, accountability, circulars, pledges] = await Promise.all([
        supabase.from("school_schedules").select("id", { count: "exact", head: true }),
        supabase.from("student_accountability").select("id", { count: "exact", head: true }),
        supabase.from("circulars").select("id", { count: "exact", head: true }),
        supabase.from("student_pledges").select("id", { count: "exact", head: true }),
      ]);
    ...
    mutationFn: async () => {
      ...
      if (active === "schedules") {
        const { error } = await supabase
          ...
          .insert({ title: title.trim(), details: details || null, schedule_date: date || null });
        if (error) throw new Error(error.message);
        ...
        const { error } = await supabase
          .from("circulars")
          .insert({ title: title.trim(), body: details || null, issued_date: date || today });
        if (error) throw new Error(error.message);
      } else {
  ...
  if (counts.isLoading) return <LoadingCards count={4} />;
